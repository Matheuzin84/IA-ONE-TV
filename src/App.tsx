...
import { Send, Bot, User, Loader2, Tv, Info, MessageCircle, Settings, HelpCircle, Copy, Check, Trash2, ExternalLink } from 'lucide-react';
...
export default function App() {
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                <div key={plan.name} className={`bg-surface p-6 rounded-xl border-t-4 ${plan.color} shadow-xl hover:scale-105 transition-transform`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                <button 
                  ...
                  className="flex items-start gap-4 p-6 bg-surface rounded-xl border border-white/5 hover:border-primary/50 transition-all text-left group active:scale-95"
                >
      ...
      case 'settings':
        return (
          <div className="flex-1 overflow-y-auto p-8 space-y-8">
            ...
            <div className="max-w-xl bg-surface p-8 rounded-2xl border border-white/5">
            ...
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center max-w-2xl mx-auto space-y-8">
                <motion.div 
                  ...
                  className="bg-surface p-10 rounded-3xl shadow-2xl border border-white/5"
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      ...
                      <button
                        ...
                        className="flex items-center gap-3 p-5 bg-surface-light hover:bg-primary border border-white/5 rounded-xl text-sm font-bold transition-all group active:scale-95"
                      >
                    ...
                    <div className="relative group">
                      <div className={`p-5 rounded-2xl shadow-xl ${
                        message.role === 'user' 
                          ...
                          : 'bg-surface text-text-main border border-white/5 rounded-tl-none'
                      }`}>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="flex gap-3 max-w-[85%]">
                  ...
                  <div className="bg-surface p-5 rounded-2xl border border-white/5 rounded-tl-none shadow-xl flex items-center gap-3">
  ...
  return (
    <div className="flex h-screen bg-background font-sans text-text-main overflow-hidden">
      {/* Sidebar */}
      <aside className="hidden md:flex w-72 bg-surface border-r border-white/5 flex-col">
        <button 
          onClick={handleGoHome}
          className="p-8 border-b border-white/5 text-left hover:bg-white/5 transition-colors group"
        >
          <h1 className="text-4xl font-display text-primary tracking-tighter group-hover:scale-105 transition-transform">ONEFLIX</h1>
          <p className="text-[10px] text-text-muted font-bold tracking-[0.2em] mt-1">ASSISTENTE PRO</p>
        </button>
        ...
        <div className="p-6 border-t border-white/5 space-y-2">
          <button
            ...
            className="w-full flex items-center gap-4 px-5 py-4 text-sm font-bold text-red-500 hover:bg-red-500/10 rounded-xl transition-all active:scale-95"
          >
            <Trash2 className="w-6 h-6" />
            Limpar Chat
          </button>
      ...
      <main className="flex-1 flex flex-col relative">
        ...
        <header className="md:hidden bg-surface border-b border-white/5 p-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setCurrentView('chat')} className={`p-3 rounded-lg ${currentView === 'chat' ? 'bg-primary' : 'bg-surface-light'} active:scale-95`}>
              <MessageCircle className="w-6 h-6" />
            </button>
            <button onClick={() => setCurrentView('plans')} className={`p-3 rounded-lg ${currentView === 'plans' ? 'bg-primary' : 'bg-surface-light'} active:scale-95`}>
        ...
        {currentView === 'chat' && (
          <header className="hidden md:flex bg-background/50 backdrop-blur-xl border-b border-white/5 p-6 items-center justify-between sticky top-0 z-10">
        ...
        {currentView === 'chat' && (
          <div className="p-6 md:p-8 bg-background border-t border-white/5">
            <div className="max-w-5xl mx-auto relative">
              <textarea
                ...
                className="w-full bg-surface border border-white/10 rounded-2xl py-5 pl-6 pr-16 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all resize-none shadow-2xl"
              />
              <button
                onClick={() => handleSend()}
